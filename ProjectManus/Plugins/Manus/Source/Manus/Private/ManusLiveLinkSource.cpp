			...
			for (int BoneIndex = 0; BoneIndex < PolygonSkeletonData.Bones.Num(); BoneIndex++)
			{
				...
				if (PolygonSkeletonData.Bones[BoneIndex].Validity)
				{
					...
					else
					{
						...
						{
						case EBoneAxis::BA_X: RefScale.Z *= FinalStretchScale; break; // Use Manus coordinate system X -> Z
						case EBoneAxis::BA_Y: RefScale.X *= FinalStretchScale; break; // Use Manus coordinate system Y -> X
						case EBoneAxis::BA_Z: RefScale.Y *= FinalStretchScale; break; // Use Manus coordinate system Z -> Y
						}
